...
console.log("[ROUTES] shipping-options aliases registered");
...
// ============================================================
// Marketplace — Acheteur: reprise paiement & annulation (TOP-LEVEL)
// ============================================================
...
// Shipping options (top-level registration)
app.get("/api/market/partners/:partnerId/shipping-options", async (req, res) => {
  try {
    const { partnerId } = req.params;
    if (!partnerId) return res.status(400).json({ error: "partnerId requis" });
...
console.log("[ROUTES] shipping-options registered top-level");
...
app.patch("/api/market/partners/:partnerId", bodyParser.json(), async (req, res) => {
  try {
    ...
    if (patch.is_open !== undefined) update.is_open = patch.is_open === true;
