...
function formatMinorAmount(amount, currency) {
  ...
  if (cur === "xaf") return `${amount} XAF`;
  // EUR/USD/CAD have 2 decimals
...
async function getPaymentSnapshot({ eventId, userId }) {
  ...
  const priceAmount = typeof ev?.price_amount === "number" ? ev.price_amount : null;
  const currency = ev?.currency ? String(ev.currency).toLowerCase() : null;

  // Gratuit si pas de price_amount (ou 0)
  ...
  const amountTotal = typeof pay?.amount_total === "number" ? pay.amount_total : priceAmount;
  const amountPaid = typeof pay?.amount_paid === "number" ? pay.amount_paid : 0;
  const cur = pay?.currency ? String(pay.currency).toLowerCase() : currency;
  const remaining = Math.max(amountTotal - amountPaid, 0);

  let paymentStatus = "unpaid";
  if (pay?.status === "paid") paymentStatus = "paid";
  else if (pay?.status === "deposit_paid") paymentStatus = "deposit_paid";
...
router.delete("/qrcode/:id", async (req, res) => {
  try {
    ...
    const { error: delErr } = await supabase.from("event_qrcodes").delete().eq("id", id).eq("user_id", user_id);
...
// Vérification via secret admin (compat)
router.get("/qrcode/verify", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("event_qrcodes")
      .select("id, status, event_id, user_id, evenements:event_id(title, date, location, price_amount, currency)")
    ...
    const isPaid = payment.payment_status === "paid" || payment.payment_status === "free";

    if (!isPaid) {
      return res.json({
        valid: true,
        canEnter: false,
        message: `QR code valide — ${payment.payment_status === "deposit_paid" ? "ACOMPTE PAYÉ" : "DOIT PAYER"}`,
        event: data.evenements || null,
        payment,
        recommended_lane: "cashier",
      });
    }
...
router.get("/qrcode/verify-jwt", async (req, res) => {
  try {
    ...
    const remainingLabel =
      payment.amount_remaining != null && payment.currency
        ? formatMinorAmount(payment.amount_remaining, payment.currency)
        : null;

    const isPaid = payment.payment_status === "paid" || payment.payment_status === "free";
    if (!isPaid) {
      const label = payment.payment_status === "deposit_paid" ? "ACOMPTE PAYÉ" : "DOIT PAYER";
      const msg = remainingLabel ? `QR code valide — ${label} — reste ${remainingLabel}` : `QR code valide — ${label}`;
