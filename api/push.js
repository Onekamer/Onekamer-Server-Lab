...
// Relais appelé par Supabase (payload déjà formé)
...
// Webhook compatible avec les triggers existants (Supabase -> Render)
// Remplace l'ancien chemin OneSignal par un relais Web Push natif.
router.post("/supabase-notification", async (req, res) => {
  ...
  try {
    ...
    if (record) {
      // Format Supabase (row new) envoyé par Edge Functions / http_request
      ...
      // Format générique (payload déjà pré-construit)
      const body = req.body || {};
